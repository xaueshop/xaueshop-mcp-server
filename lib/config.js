  ...
  return {
    ...
    tokens: {
      XAUT: {
        ...
        decimals: parseInt(process.env.XAUT_TOKEN_DECIMALS ?? String(XAUT_DECIMALS_DEFAULT), 10),
      },
      XAUE: {
        address:  resolveAddress('XAUE_TOKEN_ADDRESS', XAUE_TOKEN_MAINNET),
        decimals: parseInt(process.env.XAUE_TOKEN_DECIMALS ?? String(XAUE_DECIMALS_DEFAULT), 10),
      },
    },
    token_rules: {
    ...
    event_scan: {
      default_lookback_blocks: parseInt(process.env.XAUE_EVENT_LOOKBACK_BLOCKS ?? '200000', 10),
    },
  };
}
